<template>
  <div>
        ...
        <div class="make_plan_box">
          <div class="make_plan_item van-hairline--bottom">
            <van-row type="flex" justify="space-between">
              ...
              <van-col span="12" class="">
                <van-field class="make_plan_input" v-model="money" placeholder="还款总金额" clearable maxlength="9"
                           minlength="0" type="number" right-icon="edit" input-align="right" @input="moneyInput()"/>
              </van-col>
            </van-row>
          </div>
          <div class="make_plan_item">
            <van-row type="flex" justify="space-between">
              ...
              <van-col span="12" class="">
                <van-field class="make_plan_input" v-model="autoreservedAmount" placeholder="信用卡可用余额" clearable
                           type="number" right-icon="edit" input-align="right"/>
              </van-col>
            </van-row>
          ...
          <div @click="createTask()" class="make_plan_btn theme_btn color_fff">制定计划</div>
        </div>
      </div>
    </div>
    <!-- 日期选择器 -->
    <van-popup v-model="planDateTrueFalseBy" round position="bottom" :style="{ height: '63%' }">
      ...
      <div class="calendar">
        ...
        <div class='content'>
          ...
          <div>五</div>
          <div>六</div>
          <div v-for="(item,index) in allArr" :key="index" :class="item.day >=newDay ? '' : 'gray'"
    ...
    <!--                  :orderCode="orderCode" :type="type" v-on:createTask="createTask"/>-->

    <city ref="verificationMethod"/>
  ...
  methods: {
    ...
    authOnShow(val) {
      if (val == 2 && this.autoreservedAmount < this.minBalance) {
        ...
        if (val == 2) {
          ...
          this.budget = Math.ceil(count / this.dayRepaymentCount)
    ...
    createTask() {
      ...
      let params = {
        ...
        executeDate: date,
        isNotPoint: this.isNotPointCheck,
        dayRepaymentCount: this.dayRepaymentCount,
        provinceCode: this.provinceCode,
        cityCode: this.cityCode
      }
      creditcardTaskNew(params).then(res => {
